...
<html lang="en">
  ...
  <body>
	...
	List<Books> books = bookDao.getBooks(search); 
	%>
    ...
    <a href="./Archive.jsp"><div class="section3 menu-section">
              ...
              <div class="col-lg-3 note">
                <div class="note-head">
                    <% if(note.getWeightage().equals("High")){ %>
                        ...
                        <% } else if(note.getWeightage().equals("Moderate")){ %>
                            <div class="bookmark">
                                <img src="./resources/bookmark-check.svg" alt="" height="25px" width="25px">
                            </div>
                        <% } else {%>
                        	<div class="bookmark">
                                <img src="./resources/bookmark-dash.svg" alt="" height="25px" width="25px">
                            </div>
                        <% } %>
                    <div class="pinlogo">
                ...
                <form method="post" action="pin">
                    ...
                    <button type="submit" class="notebutton">
                        <img
                          src="./resources/pin-angle-fill.svg"
                          height="20px"
                          width="20px"
                        />
                      </button>
                ...
                <p><%=note.getTopic() %></p>
                <span><%=note.getSubName()%></span>
                <span><%=note.getUnit() %></span>
                <div class="operation">
                  ...
                  <% if(isLabelListEmpty == false){ %>
					...
					<select name="label" class="select-label">
						<%
						for (String list : lists) {
							boolean resultLabel = dao.isLabelled(username, note.getFileName(), list);
        ...
        <div class="col-lg-3 note">
            ...
            <p><%=prac.getTopic() %></p>
            <span><%=prac.getSubName()%></span>
            <span><%=prac.getUnit() %></span>
            <div class="operation">
              <form action="download" method="post">
                <input type="hidden" name="subName" value="<%=prac.getSubName()%>">
                <input type="hidden" name="fileName"
                    value="<%=prac.getFileName()%>"> 
                <button type="submit" class="notebutton">
                  <img
                    src="./resources/download.svg"
                    alt=""
              ...
              <% if(isLabelListEmpty == false){ %>
					...
					<select name="label" class="select-label">
						<%
						for (String list : lists) {
							boolean resultLabel = dao.isLabelled(username, prac.getFileName(), list);
    ...
    %>
        ...
        <div class="col-lg-3 note">
            <div class="note-head">
                <div class="pinlogo">
                    <% boolean result = dao.isPinned(username, book.getFileName());
                if(result){
            %>
            ...
            <div class="operation">
              <form action="download" method="post">
                <input type="hidden" name="subName" value="<%=book.getSubName()%>">
                <input type="hidden" name="fileName"
                    value="<%=book.getFileName()%>"> 
                <button type="submit" class="notebutton">
                  <img
                    src="./resources/download.svg"
                    alt=""
              ...
              <% if(isLabelListEmpty == false){ %>
                  	<form action="pin" method="post" class="labelform">
                    <input type="hidden" name="userName" value="<%=username%>"> <input
						type="hidden" name="fileName" value="<%=book.getFileName()%>">
					<input type="hidden" name="action" value="pinlabel">
					<select name="label" class="select-label">
						<%
						for (String list : lists) {
							boolean resultLabel = dao.isLabelled(username, book.getFileName(), list);
