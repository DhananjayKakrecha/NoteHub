...
<body>
	...
	String action = (String) request.getSession().getAttribute("utype");

	List<String> values = new ArrayList<>();
	String notestype = "";
	int id = 0;
	if (action.equals("unotes")) {
		Notes notes = (Notes) request.getSession().getAttribute("nobject");
		values.add(notes.getTopic());
		values.add(notes.getSubName());
		values.add(notes.getWeightage());
		values.add(notes.getUnit());
		values.add(notes.getFileName());
		id = notes.getNid();
		notestype = "Notes";
	} else if (action.equals("upracs")) {
		Practicals pracs = (Practicals) request.getSession().getAttribute("pobject");
		values.add(pracs.getTopic());
		values.add(pracs.getSubName());
		values.add(pracs.getWeightage());
		values.add(pracs.getUnit());
		values.add(pracs.getFileName());
		id = pracs.getPid();
		notestype = "Practicals";
	} else if (action.equals("ubooks")) {
		Books books = (Books) request.getSession().getAttribute("ubooks");
		values.add(books.getTopic());
		values.add(books.getSubName());
		values.add(books.getWeightage());
		values.add(books.getUnit());
		values.add(books.getFileName());
		id = books.getBid();
		notestype = "Books";
	}
	%>
					...
					<form class="row g-3" action="upload" method="post"
						...
						<div class="col-md-6">
							...
							<input class="form-control" list="datalistOptions"
								...
								value="<%=values.get(1)%>" required readonly />
						...
						<div class="col-md-6">
							<label for="notesType" class="form-label">Notes Type</label> <input
								type="text" class="form-control" id="inputAddress"
								name="notetype" value="<%=notestype%>" required readonly />
						</div>
						<div class="col-md-4">
							<label for="weightage" class="form-label">Weightage</label> <select
								id="inputState" class="form-select" name="weightage" required>
								<option selected><%=values.get(2)%></option>
								<option>High</option>
						...
						<div class="col-md-4">
							<label for="unit" class="form-label">Unit</label> <select
								...
								<option selected><%=values.get(3)%></option>
						...
						<div class="col-12">
							<label for="fileName" class="form-label">Uploaded File</label> <input
								type="text" class="form-control" id="inputAddress"
								name="fileName" value="<%=values.get(4)%>" readonly />
						</div>
						<div class="div">
							<input type="hidden" name="id" value="<%=id%>"> <input
								type="hidden" name="action" value="updateNotes">
